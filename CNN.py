...
def train(model,epoch, X_train, Y_train, train_losses, criterion):
    model.train()
    tr_loss = 0
...
def main():
    ...
    #     if loss_train/len(train_loader)<loss_valid/len(train_loader)-0.3:
    #         break
    # plt.plot(range(len(train_loss)),train_loss,'o-')
    # plt.plot(range(len(train_loss)),valid_loss,'o-')
    # plt.savefig("loss.png")
    print("test performance on PRIORI Emotion dataset")




    print("test performance on R21 dataset")
    with torch.no_grad():
        Y_21_out=model(torch.from_numpy(X_R21)).numpy()
        # print(Y_out.shape)
    Y_pred=np.zeros(Y_21_out.shape[0])
    for i, pred in enumerate(Y_21_out):
        Y_pred[i]=np.argmax(pred)
    print(metrics.accuracy_score(Y_R21,Y_pred))
    print(metrics.recall_score(Y_R21,Y_pred,average="macro"))
